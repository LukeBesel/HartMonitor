...
export default function Pricing() {
  ...
  const cta = (key: string) => (key === 'enterprise' ? 'Contact sales' : 'Start free — early access');
    ...
    <div className="bg-[#060911] text-white antialiased min-h-screen overflow-x-hidden">
      ...
      <section className="relative pt-36 pb-12 md:pt-44 text-center px-6">
        ...
        <Reveal className="relative">
          ...
          <p className="mt-6 text-lg text-gray-400 max-w-2xl mx-auto">We're in early access: everything below is free right now — every module, unlimited apps and dashboards, no credit card. These are the prices that will apply when paid plans launch.</p>
          <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/5 text-sm font-medium text-gray-300">
            ...
            Early access — the entire platform is free right now
          </div>
        </Reveal>
      </section>
              ...
              <Reveal key={key} delay={i * 90}>
                <div className={`relative h-full rounded-2xl p-8 border flex flex-col ${featured ? 'border-pink-500/40 bg-gradient-to-b from-pink-500/[0.1] to-transparent' : 'border-white/10 bg-white/[0.03]'}`}>
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    {key === 'enterprise' ? "Let's talk about your needs" : 'Free during early access — no card, no limits'}
                  </p>
