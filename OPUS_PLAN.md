...
- **Shipped** through PR #24 (Apps-first wave: showcase library, in-depth app pages, builder-first training coach, team alert system, freshness stamps, dashboard filters, theme fixes) and PR #25 (department filters across the management screens; five modules that 500'd on every database; dark-mode chip contrast; setup checklist no longer hiding half the workspaces).
- **Test baseline**: 264 backend / 238 frontend, typecheck + build clean. Two standing guards worth knowing about: `backend/test/schema-drift.test.js` walks every INSERT/UPDATE against the live schema AND presses all 34 create endpoints for 5xx; `backend/test/partial-update.test.js` pins the absent-vs-cleared contract.
...
**Still open:** the *render-side* half — page config maps must cover their table's CHECK set (Kaizen's `STATUS_FILTERS` offers `reviewing` but not the CHECK-legal `under_review`, so rows stored under the alias are unreachable by the chip). The crash risk itself is handled — every page map has a fallback (`catOf`/`statusOf`) — so this is a reachability bug, not a 500. Worth a test asserting page-maps ⊇ CHECK-sets.
